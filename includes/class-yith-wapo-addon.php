...
if ( ! class_exists( 'YITH_WAPO_Addon' ) ) {
	...
	class YITH_WAPO_Addon {
		...
		public function get_option_price_html( $index, $currency = false, $product = null ) {
			...
			if ( 'free' !== $price_method ) {
				...
				if ( '' !== $option_price ) {
					...
					if ( 'select' === $this->get_type() ) {
						if ( '' !== $option_price_sale && floatval( $option_price_sale ) >= 0 ) {
							$html_price = '(<span class="sign ' . $sign_class . '">' . $sign . '</span>' . wc_price( $option_price_sale, array( 'currency' => $currency ) ) . ')';
						} else {
							$html_price = '(<span class="sign ' . $sign_class . '">' . $sign . '</span>' . wc_price( $option_price, array( 'currency' => $currency ) ) . ')';
						}
					} elseif ( '' !== $option_price_sale && floatval( $option_price_sale ) >= 0 ) {
		...
		public function get_default_sale_price( $index ) {
			return $this->get_option( 'price_sale', $index );
		}
		...
		/**
		 *  Get add-on current price.
		 *
		 * @param int     $index Option index.
		 * @param boolean $calculate_taxes Calculate the taxes of the prices.
		 * @return float
		 */
		public function get_total_price( $index, $calculate_taxes = true, $product = null ) {

			$fixed_price = $this->get_price( $index, $calculate_taxes );
			...
			if ( 'free' !== $price_method ) {
				...
				if ( 'discount' === $price_method ) { // Discount price of linked product.
					$option_discount_value = floatval( $fixed_price );
					if ( 'percentage' === $price_type ) {
						$option_price = $product_price_addon - ( ( $product_price_addon / 100 ) * $option_discount_value );
					} else {
						$option_price = $product_price_addon - $option_discount_value;
					}
				}
		...
		/**
		 * Check if the add-on is sell individually.
		 *
		 * @return false
		 */
		public function is_sell_individually() {
			return false;
		}

		/**
		 * Get the price method of the specific add-on.
		 *
		 * @param int $index The index of the add-on.
		 * @return mixed|string|null
		 */
		public function get_price_method( $index ) {
			return $this->get_option( 'price_method', $index, 'free', false );
		}

		/**
		 * Get the price type of the specific add-on.
		 *
		 * @param int $index The index of the add-on.
		 * @return mixed|string|null
		 */
		public function get_price_type( $index ) {
			return $this->get_option( 'price_type', $index, 'fixed', false );
		}
	}
